...
const RiskAssessmentStats = ({ stats, onUpdateFilters, districts, resetFilters }) => {
  ...
  useEffect(() => {
    ...
    if (searchParams?.get('bid')) setBid(searchParams?.get('bid'));
    else setBid('');
  }, [searchParams]);
    ...
    if (t)
      return [
        ...
        <Typography variant='body2' key={key.translation}>
          {t(`routes.statistics.num_${key.translation}`)}:{' '}
          <b>{stats?.keyNumbers ? stats?.keyNumbers[key.statKey] : 0} </b>
        </Typography>
      ));
  }, [t, stats]);
  ...
  return (
    <Grid container spacing={4} flexDirection='column'>
      ...
      <Grid item xs={12} container flexDirection='row' spacing={3} style={{ paddingTop: 0 }}>
        ...
        <Grid item>
          <TextField
            label={t('common.strings.bid')}
            value={bid}
            onChange={(e) => setBid(e.target.value)}
            onBlur={() => onUpdateFilters({ bid })}
          />
        </Grid>
      </Grid>
...
export default React.memo(RiskAssessmentStats);
